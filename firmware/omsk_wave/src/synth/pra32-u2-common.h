...
static INLINE uint8_t low_byte(uint16_t x) { return x & 0xFF; }

static INLINE uint8_t high_byte(uint16_t x) { return (x >> 8) & 0xFF; }
...
static INLINE int32_t minimum(int32_t value_0, int32_t value_1) {
  volatile int32_t result =
      (value_0 < value_1) * value_0 + (value_0 >= value_1) * value_1;
  return result;
}
...
static INLINE int32_t clamp(int32_t value, int32_t minimum_value,
