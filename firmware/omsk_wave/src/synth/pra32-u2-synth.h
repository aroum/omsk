...
#include "../env/pra32-u2-eg.h"
#include "../filt/pra32-u2-filter.h"
#include "../fx/pra32-u2-chorus-fx.h"
#include "../fx/pra32-u2-delay-fx.h"
#include "../fx/pra32-u2-panner.h"
#include "../gen/pra32-u2-amp.h"
#include "../gen/pra32-u2-lfo.h"
#include "../gen/pra32-u2-noise-gen.h"
#include "../gen/pra32-u2-osc.h"
#include "../tables/pra32-u2-program-table.h"
...
static uint8_t s_program_table_parameters[] = {
    OSC_1_WAVE,     OSC_1_SHAPE,     OSC_1_MORPH,
    MIXER_SUB_OSC,

    OSC_2_WAVE,     OSC_2_COARSE,    OSC_2_PITCH,
    MIXER_OSC_MIX,

    FILTER_CUTOFF,  FILTER_RESO,     FILTER_EG_AMT,
    FILTER_KEY_TRK,

    EG_ATTACK,      EG_DECAY,        EG_SUSTAIN,
    EG_RELEASE,

    EG_OSC_AMT,     EG_OSC_DST,      VOICE_MODE,
    PORTAMENTO,

    LFO_WAVE,       LFO_RATE,        LFO_DEPTH,
    LFO_FADE_TIME,

    LFO_OSC_AMT,    LFO_OSC_DST,     LFO_FILTER_AMT,
    AMP_GAIN,

    AMP_ATTACK,     AMP_DECAY,       AMP_SUSTAIN,
    AMP_RELEASE,

    FILTER_MODE,    EG_AMP_MOD,      REL_EQ_DECAY,
    P_BEND_RANGE,

    BTH_FILTER_AMT, BTH_AMP_MOD,     EG_VEL_SENS,
    AMP_VEL_SENS,

    AFT_T_LFO_AMT,  VOICE_ASGN_MODE, PAN,

    CHORUS_MIX,     CHORUS_RATE,     CHORUS_DEPTH,

    DELAY_LEVEL,    DELAY_TIME,      DELAY_FEEDBACK,
    DELAY_MODE,
};

static uint8_t s_program_table_panel_parameters[] = {
    PANEL_SCALE,    PANEL_TRANSPOSE, PANEL_PLAY_MODE, PANEL_MIDI_CH,

    PANEL_PLAY_PIT, PANEL_PLAY_VELO, PANEL_PIT_OFST,

    SEQ_PITCH_0,    SEQ_PITCH_1,     SEQ_PITCH_2,     SEQ_PITCH_3,

    SEQ_PITCH_4,    SEQ_PITCH_5,     SEQ_PITCH_6,     SEQ_PITCH_7,

    SEQ_VELO_0,     SEQ_VELO_1,      SEQ_VELO_2,      SEQ_VELO_3,

    SEQ_VELO_4,     SEQ_VELO_5,      SEQ_VELO_6,      SEQ_VELO_7,

    SEQ_TEMPO,      SEQ_CLOCK_SRC,   SEQ_GATE_TIME,   SEQ_NUM_STEPS,

    SEQ_MODE,       SEQ_ACT_STEPS,   SEQ_PIT_OFST,    SEQ_STEP_NOTE,

    SEQ_ON_STEPS,   SEQ_TRX_ST_SP,
...
public:
  PRA32_U2_Synth()
      ...
      : m_osc(), m_filter(), m_amp(), m_panner(), m_noise_gen(), m_lfo(),
        m_eg(), m_chorus_fx(), m_delay_fx()

        ,
        m_count(0xFFFFFFFFu)

        ,
        m_note_queue(), m_note_on_number(), m_note_on_count(),
        m_note_on_total_count(), m_last_note_on_index(3), m_sustain_pedal(),
        m_voice_mode(0xFF), m_voice_asgn_mode(1)

        ,
        m_output_error(), m_portamento()

        ,
        m_chorus_mode()

        ,
        m_eg_osc_amt(), m_eg_osc_dst(), m_lfo_osc_amt(), m_lfo_osc_dst()

        ,
        m_controller_value_eg_attack(0), m_controller_value_eg_decay(127),
        m_controller_value_eg_sustain(0), m_controller_value_eg_release(0),
        m_controller_value_amp_attack(0), m_controller_value_amp_decay(127),
        m_controller_value_amp_sustain(0), m_controller_value_amp_release(0),
        m_controller_value_eg_amp_mod(0), m_controller_value_rel_eq_decay(0)

        ,
        m_program_number_to_write(8), m_wr_prog_to_flash_cc_value(0),
        m_sp_prog_chg_cc_values(), m_current_controller_value_table(),
        m_program_table(), m_program_table_panel()
  ...
  /* INLINE */ void __not_in_flash_func(initialize)() {
    std::memcpy(m_program_table[OSC_1_WAVE], g_preset_table_OSC_1_WAVE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[MIXER_SUB_OSC], g_preset_table_MIXER_SUB_OSC,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[OSC_1_SHAPE], g_preset_table_OSC_1_SHAPE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[OSC_1_MORPH], g_preset_table_OSC_1_MORPH,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[OSC_2_WAVE], g_preset_table_OSC_2_WAVE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[MIXER_OSC_MIX], g_preset_table_MIXER_OSC_MIX,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[OSC_2_COARSE], g_preset_table_OSC_2_COARSE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[OSC_2_PITCH], g_preset_table_OSC_2_PITCH,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[FILTER_CUTOFF], g_preset_table_FILTER_CUTOFF,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[FILTER_RESO], g_preset_table_FILTER_RESO,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[FILTER_EG_AMT], g_preset_table_FILTER_EG_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[FILTER_KEY_TRK], g_preset_table_FILTER_KEY_TRK,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[EG_ATTACK], g_preset_table_EG_ATTACK,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_DECAY], g_preset_table_EG_DECAY,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_SUSTAIN], g_preset_table_EG_SUSTAIN,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_RELEASE], g_preset_table_EG_RELEASE,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[EG_OSC_AMT], g_preset_table_EG_OSC_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_OSC_DST], g_preset_table_EG_OSC_DST,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[VOICE_MODE], g_preset_table_VOICE_MODE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[PORTAMENTO], g_preset_table_PORTAMENTO,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[LFO_WAVE], g_preset_table_LFO_WAVE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[LFO_FADE_TIME], g_preset_table_LFO_FADE_TIME,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[LFO_RATE], g_preset_table_LFO_RATE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[LFO_DEPTH], g_preset_table_LFO_DEPTH,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[LFO_OSC_AMT], g_preset_table_LFO_OSC_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[LFO_OSC_DST], g_preset_table_LFO_OSC_DST,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[LFO_FILTER_AMT], g_preset_table_LFO_FILTER_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[AMP_GAIN], g_preset_table_AMP_GAIN,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[AMP_ATTACK], g_preset_table_AMP_ATTACK,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[AMP_DECAY], g_preset_table_AMP_DECAY,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[AMP_SUSTAIN], g_preset_table_AMP_SUSTAIN,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[AMP_RELEASE], g_preset_table_AMP_RELEASE,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[FILTER_MODE], g_preset_table_FILTER_MODE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[P_BEND_RANGE], g_preset_table_P_BEND_RANGE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_AMP_MOD], g_preset_table_EG_AMP_MOD,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[REL_EQ_DECAY], g_preset_table_REL_EQ_DECAY,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[BTH_FILTER_AMT], g_preset_table_BTH_FILTER_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[BTH_AMP_MOD], g_preset_table_BTH_AMP_MOD,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[EG_VEL_SENS], g_preset_table_EG_VEL_SENS,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[AMP_VEL_SENS], g_preset_table_AMP_VEL_SENS,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[AFT_T_LFO_AMT], g_preset_table_AFT_T_LFO_AMT,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[VOICE_ASGN_MODE],
                g_preset_table_VOICE_ASGN_MODE, sizeof(m_program_table[0]));
    std::memcpy(m_program_table[PAN], g_preset_table_PAN,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[CHORUS_MIX], g_preset_table_CHORUS_MIX,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[CHORUS_RATE], g_preset_table_CHORUS_RATE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[CHORUS_DEPTH], g_preset_table_CHORUS_DEPTH,
                sizeof(m_program_table[0]));

    std::memcpy(m_program_table[DELAY_LEVEL], g_preset_table_DELAY_LEVEL,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[DELAY_MODE], g_preset_table_DELAY_MODE,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[DELAY_FEEDBACK], g_preset_table_DELAY_FEEDBACK,
                sizeof(m_program_table[0]));
    std::memcpy(m_program_table[DELAY_TIME], g_preset_table_DELAY_TIME,
                sizeof(m_program_table[0]));

    for (uint32_t i = 0;
         i < sizeof(m_program_table_panel) / sizeof(m_program_table_panel[0]);
         ++i) {
    ...
    for (uint32_t program_number = 0; program_number <= USER_PROGRAM_NUMBER_MAX;
         ++program_number) {
      if ((EEPROM.read(1024 + program_number * 128) == 'U') &&
          (EEPROM.read(1024 + program_number * 128 + 1) == program_number)) {
        for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
                                     sizeof(s_program_table_parameters[0]);
             ++i) {
          uint32_t control_number = s_program_table_parameters[i];
          uint8_t value_read =
              EEPROM.read(1024 + program_number * 128 + control_number);
          if (value_read < 128) {
    ...
    if ((EEPROM.read(0) == 'P') && (EEPROM.read(1) == 0)) {
      for (uint32_t i = 0; i < sizeof(s_program_table_panel_parameters) /
                                   sizeof(s_program_table_panel_parameters[0]);
           ++i) {
  ...
  /* INLINE */ void __not_in_flash_func(note_on)(uint8_t note_number,
                                                 uint8_t velocity) {
    if (velocity == 0) {
    ...
    if ((m_voice_mode == VOICE_MONOPHONIC) || (m_voice_mode == VOICE_LEGATO) ||
        (m_voice_mode == VOICE_LEGATO_PORTA)) {
  ...
  void all_notes_off() {
    ...
    m_note_on_number[3] = NOTE_NUMBER_INVALID;
    for (uint8_t i = 0;
         i < (sizeof(m_note_on_count) / sizeof(m_note_on_count[0])); ++i) {
      m_note_on_count[i] = 0;
    }
  ...
  /* INLINE */ void
  __not_in_flash_func(control_change)(uint8_t control_number,
                                      uint8_t controller_value) {
    m_current_controller_value_table[control_number] = controller_value;

#if defined(PRA32_U2_USE_CONTROL_PANEL)
    PRA32_U2_ControlPanel_on_control_change(control_number);
#endif // defined(PRA32_U2_USE_CONTROL_PANEL)
    ...
    case FILTER_MODE: {
      uint8_t mode = ((static_cast<uint32_t>(controller_value) * 8) + 127) / 254;
      if (mode > 4) mode = 4;
      m_filter[0].set_filter_mode(mode);
      m_filter[1].set_filter_mode(mode);
      m_filter[2].set_filter_mode(mode);
      m_filter[3].set_filter_mode(mode);
    } break;
    ...
    case FILTER_MIX:
      m_filter[0].set_mix(controller_value);
      m_filter[1].set_mix(controller_value);
      m_filter[2].set_mix(controller_value);
      m_filter[3].set_mix(controller_value);
      break;
    ...
    case MONO_MODE_ON:
    case POLY_MODE_ON:
      all_notes_off(); // Strictly speaking, this is a violation of MIDI 1.0
                       // Specification...
      break;
  ...
  /* INLINE */ void __not_in_flash_func(after_touch_poly)(uint8_t note_number,
                                                          uint8_t pressure) {
  ...
  /* INLINE */ void
  __not_in_flash_func(program_change)(uint8_t program_number) {
    if (program_number > PROGRAM_NUMBER_MAX) {
      if ((program_number == 128) || (program_number == 129)) {
        for (uint32_t i = 0;
             i < sizeof(s_program_table_panel_parameters) /
                     sizeof(s_program_table_panel_parameters[0]);
             ++i) {
          uint32_t control_number = s_program_table_panel_parameters[i];
          control_change(
              control_number,
              m_program_table_panel[program_number - 128][control_number]);
        }
      }
      return;
    }
    ...
    for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
      ...
      control_change(control_number,
                     m_program_table[control_number][program_number]);
    }
  }

  /* INLINE */ void __not_in_flash_func(write_parameters_to_program)(
      uint8_t program_number_to_write) {
    if ((program_number_to_write > USER_PROGRAM_NUMBER_MAX) &&
        (program_number_to_write != 128)) {
      return;
    }
    ...
    if (program_number_to_write <= USER_PROGRAM_NUMBER_MAX) {
      for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
           ...
           ++i) {
        uint32_t control_number = s_program_table_parameters[i];
        m_program_table[control_number][program_number_to_write] =
            m_current_controller_value_table[control_number];
      }
    }
    ...
    if (program_number_to_write == 128) {
      for (uint32_t i = 0; i < sizeof(s_program_table_panel_parameters) /
           ...
           ++i) {
        uint32_t control_number = s_program_table_panel_parameters[i];
        m_program_table_panel[0][control_number] =
            m_current_controller_value_table[control_number];
      }
    }
#endif // defined(PRA32_U2_USE_CONTROL_PANEL)
    ...
    if (program_number_to_write <= PROGRAM_NUMBER_MAX) {
      for (uint32_t i = 0; i < sizeof(s_program_table_parameters) /
           ...
           ++i) {
        uint32_t control_number = s_program_table_parameters[i];
        EEPROM.write(1024 + program_number_to_write * 128 + control_number,
                     m_current_controller_value_table[control_number]);
      }

      EEPROM.write(1024 + program_number_to_write * 128, 'U');
      EEPROM.write(1024 + program_number_to_write * 128 + 1,
                   program_number_to_write);
    }
  ...
  /* INLINE */ int16_t
  __not_in_flash_func(process)(int16_t &right_output_int16) {
    ++m_count;
    ...
    case 0x00: {
      ...
      m_filter[0].process_at_low_rate(m_count >> 2, m_eg[0].get_output(),
                                      lfo_output, m_osc.get_osc_pitch(0));
    ...
    case 0x01: {
      ...
      m_filter[1].process_at_low_rate(m_count >> 2, m_eg[2].get_output(),
                                      lfo_output, m_osc.get_osc_pitch(1));
    ...
    case 0x02: {
      ...
      m_filter[2].process_at_low_rate(m_count >> 2, m_eg[4].get_output(),
                                      lfo_output, m_osc.get_osc_pitch(2));
    ...
    case 0x03: {
      ...
      m_filter[3].process_at_low_rate(m_count >> 2, m_eg[6].get_output(),
                                      lfo_output, m_osc.get_osc_pitch(3));
    ...
    if (m_voice_mode == VOICE_POLYPHONIC) {
      voice_mixer_output =
          amp_output[0] + amp_output[1] + m_secondary_core_processing_result;
    } else {
    ...
    int32_t panner_output_l =
        m_panner.process(voice_mixer_output, panner_output_r);

    int32_t chorus_fx_output_r;
    int32_t chorus_fx_output_l = m_chorus_fx.process(
        panner_output_l, panner_output_r, chorus_fx_output_r);

    int32_t delay_fx_output_r;
    int32_t delay_fx_output_l = m_delay_fx.process(
        chorus_fx_output_l, chorus_fx_output_r, delay_fx_output_r);

    int32_t synth_output_r =
        clamp(delay_fx_output_r, (-(INT16_MAX << 8)), (+(INT16_MAX << 8)));
    int32_t synth_output_l =
        clamp(delay_fx_output_l, (-(INT16_MAX << 8)), (+(INT16_MAX << 8)));
    ...
    // Dithering
    right_output_int16 =
        synth_output_r_int16 + (((noise_int15 + 16384) >> 11) - 8);
    ...
    right_output_int16 =
        synth_output_r_int16 + (prev_output_error_r > s_output_error_r) * 22;
...
#if 1
    // Dithering
    int16_t synth_output_l_int16 =
        ((synth_output_l + ((noise_int15 + 16384) >> 7)) >> 8);
    int16_t synth_output_r_int16 =
        ((synth_output_r + ((noise_int15 + 16384) >> 7)) >> 8);
#else
    int16_t synth_output_l_int16 = (synth_output_l >> 8);
    int16_t synth_output_r_int16 = (synth_output_r >> 8);
#endif
  ...
  /* INLINE */ boolean __not_in_flash_func(secondary_core_process)() {
    ...
    if (m_secondary_core_processing_request == 1) {
      int16_t noise_int15 =
          static_cast<int16_t>(m_secondary_core_processing_argument);
...
private:
  ...
  INLINE void set_voice_mode(uint8_t controller_value) {
    ...
    static uint8_t voice_mode_table[6] = {
        VOICE_POLYPHONIC, VOICE_POLYPHONIC,   VOICE_MONOPHONIC,
        VOICE_MONOPHONIC, VOICE_LEGATO_PORTA, VOICE_LEGATO,
    };
#else  // defined(PRA32_U2_USE_2_CORES_FOR_SIGNAL_PROCESSING) ||
       // defined(PRA32_U2_EMULATION)
    static uint8_t voice_mode_table[6] = {
        VOICE_MONOPHONIC, VOICE_MONOPHONIC,   VOICE_MONOPHONIC,
        VOICE_MONOPHONIC, VOICE_LEGATO_PORTA, VOICE_LEGATO,
    };
#endif // defined(PRA32_U2_USE_2_CORES_FOR_SIGNAL_PROCESSING) ||
       // defined(PRA32_U2_EMULATION)
