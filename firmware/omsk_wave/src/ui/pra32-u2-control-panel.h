...
static INLINE uint8_t PRA32_U2_ControlPanel_calc_transposed_pitch(uint8_t pitch, int32_t transpose)
{
...
static INLINE boolean PRA32_U2_ControlPanel_calc_value_display(uint8_t control_target, uint8_t controller_value, char value_display_text[5])
{
  ...
  case FILTER_RESO:
    {
      uint8_t res_idx = controller_value >> 2;
      if (res_idx > 31) res_idx = 31;
      float q_val = g_vcf_res_q_lut[res_idx];
      std::sprintf(value_display_text, "%3.1f", q_val);
      result = true;
    }
    break;
  case PANEL_PIT_OFST  :
  ...
  case FILTER_MODE     :
    {
      char ary[5][5] = {" LP", " BP", " HP", " BS", " AP"};
      uint32_t index = ((static_cast<uint32_t>(controller_value) * 8) + 127) / 254;
      if (index > 4) index = 4;
      std::strcpy(value_display_text, ary[index]);
      result = true;
    }
