...
public:
  PRA32_U2_Amp()
      : m_gain_control(127), m_gain_control_effective(0),
        m_expression_control(127), m_expression_control_effective(0),
        m_gain_linear(), m_gain_mod_input(0), m_breath_mod(),
        m_breath_controller(), m_breath_gain_linear() {}
...
private:
  INLINE void update_gain_effective() {
    ...
    m_expression_control_effective +=
        (m_expression_control_effective < m_expression_control);
    m_expression_control_effective -=
        (m_expression_control_effective > m_expression_control);

    m_gain_linear =
        ((((m_gain_control_effective * m_gain_control_effective) * 16384) /
          16129) *
         (((m_expression_control_effective * m_expression_control_effective) *
           16384) /
          16129)) >>
        (14 - 2);
  }
  ...
  INLINE void update_breath_controller_effective() {
    if (m_breath_mod == 2) {
      ...
      m_breath_gain_linear =
          (((m_breath_controller * m_breath_controller) * 16384) / 16129) << 2;
    } else {
