...
#include "../tables/pra32-u2-osc-sqr-shape-table.h"
#include "../tables/pra32-u2-osc-table.h"
#include "../tables/pra32-u2-osc-wave-shape-table-0.h"
#include "../tables/pra32-u2-osc-wave-shape-table-1.h"
#include "../tables/pra32-u2-osc-wave-shape-table-2.h"
#include "../tables/pra32-u2-osc-wave-shape-table-3.h"
#include "../tables/pra32-u2-osc-wave-shape-table-4.h"
#include "../tables/pra32-u2-osc-wave-shape-table-5.h"

#if CFG_OSC_MODE == OSC_MODE_TABLE
...
public:
  PRA32_U2_Osc()
      : m_portamento_coef(), m_pitch_eg_amt(), m_pitch_lfo_amt()

        ,
        m_waveform_index(), m_waveform(), m_pitch_bend(), m_pitch_bend_range(),
        m_pitch_bend_normalized(), m_pitch_target(), m_pitch_current(),
        m_wave_table(), m_wave_table_temp(), m_freq(), m_freq_base(),
        m_freq_offset(), m_phase(), m_phase_shape_morph(), m_osc_on(),
        m_osc_level_effective(), m_osc_level()

        ,
        m_mixer_osc_mix_control(), m_mixer_osc_mix_control_effective(),
        m_osc2_pitch(), m_osc2_detune()

        ,
        m_phase_high(), m_osc1_shape_control(),
        m_osc1_shape_control_effective(), m_osc1_morph_control(),
        m_osc1_morph_control_effective(), m_osc1_shape(),
        m_osc1_shape_effective(), m_osc1_phase_modulation_frequency_ratio(),
        m_mixer_noise_sub_osc_control(),
        m_mixer_noise_sub_osc_control_effective(), m_mix_table(),
        m_shape_eg_amt(), m_shape_lfo_amt() {
    ...
    for (uint8_t i = 0; i < OSC_MIX_TABLE_LENGTH; ++i) {
      m_mix_table[i] = static_cast<int16_t>(
          sqrtf(static_cast<float>(i) / (OSC_MIX_TABLE_LENGTH - 1)) *
          (1 << 10));
    }

    set_pitch_bend_range(CFG_PITCH_BEND_RANGE_SEMITONES);
  }

  template <uint8_t N> INLINE void set_osc_waveform(uint8_t controller_value) {
  ...
  INLINE int16_t get_pitch_mod_amt_table(uint8_t controller_value) {
    static int16_t pitch_mod_amt_table[128] = {
        -30720, -30720, -24576, -18432, -15360, -12288, -10752, -9216,  -7680,
        -6144,  -5888,  -5632,  -5376,  -5120,  -4864,  -4608,  -4352,  -4096,
        -3840,  -3584,  -3328,  -3072,  -2816,  -2560,  -2304,  -2048,  -1792,
        -1536,  -1280,  -1024,  -768,   -512,   -256,   -248,   -240,   -232,
        -224,   -216,   -208,   -200,   -192,   -184,   -176,   -168,   -160,
        -152,   -144,   -136,   -128,   -120,   -112,   -104,   -96,    -88,
        -80,    -72,    -64,    -56,    -48,    -40,    -32,    -24,    -16,
        -8,     +0,     +8,     +16,    +24,    +32,    +40,    +48,    +56,
        +64,    +72,    +80,    +88,    +96,    +104,   +112,   +120,   +128,
        +136,   +144,   +152,   +160,   +168,   +176,   +184,   +192,   +200,
        +208,   +216,   +224,   +232,   +240,   +248,   +256,   +512,   +768,
        +1024,  +1280,  +1536,  +1792,  +2048,  +2304,  +2560,  +2816,  +3072,
        +3328,  +3584,  +3840,  +4096,  +4352,  +4608,  +4864,  +5120,  +5376,
        +5632,  +5888,  +6144,  +7680,  +9216,  +10752, +12288, +15360, +18432,
        +24576, +30720,
    };
  ...
  template <uint8_t N> INLINE void set_pitch_eg_amt(uint8_t controller_value) {
  ...
  template <uint8_t N> INLINE void set_pitch_lfo_amt(uint8_t controller_value) {
  ...
  INLINE void set_osc2_detune(uint8_t controller_value) {
    static int16_t m_osc2_detune_table[128] = {
        ...
        -384,  -256,  -128,  -124,  -120,  -116,  -112,  -108,  -104,  -100,
        -96,   -92,   -88,   -84,   -80,   -76,   -72,   -68,   -64,   -60,
        -56,   -52,   -48,   -44,   -40,   -36,   -32,   -28,   -24,   -20,
        -16,   -12,   -8,    -4,    +0,    +4,    +8,    +12,   +16,   +20,
        +24,   +28,   +32,   +36,   +40,   +44,   +48,   +52,   +56,   +60,
        +64,   +68,   +72,   +76,   +80,   +84,   +88,   +92,   +96,   +100,
        +104,  +108,  +112,  +116,  +120,  +124,  +128,  +256,  +384,  +512,
  ...
  template <uint8_t N> INLINE void set_portamento(uint8_t controller_value) {
    m_portamento_coef[N] = g_portamento_coef_table[controller_value];
  }

  template <uint8_t N> INLINE void note_on(uint8_t note_number) {
    uint8_t n;
  ...
  template <uint8_t N> INLINE void note_off() { m_osc_on[N] = false; }
  ...
  INLINE uint16_t get_osc_pitch(uint8_t index) {
    uint16_t shifted_pitch =
        (64 << 8) + (m_pitch_current[index] >> 16) + m_pitch_bend_normalized;
    uint16_t osc_pitch;
  ...
  template <uint8_t N> INLINE int32_t process(int16_t noise_int15) {
#if 1
...
private:
  ...
  INLINE int16_t get_wave_level(const int16_t *wave_table, uint32_t phase_24) {
    ...
    uint16_t next_weight =
        phase_16 & ((1 << (16 - OSC_WAVE_TABLE_SAMPLES_BITS)) - 1);
    int16_t curr_data = wave_table[curr_index + 0];
    int16_t next_data = wave_table[curr_index + 1];
    int16_t level = curr_data + (((next_data - curr_data) * next_weight) >>
                                 (16 - OSC_WAVE_TABLE_SAMPLES_BITS)); // lerp
    return level;
  }

  INLINE const uint16_t (*get_wave_shape_table(
      uint8_t osc1_morph_control))[OSC_WAVE_SHAPE_TABLE_LEN_X]
                                  [OSC_WAVE_SHAPE_TABLE_LEN_Y] {
    static const uint16_t(*wave_shape_table[6])[OSC_WAVE_SHAPE_TABLE_LEN_X]
                                               [OSC_WAVE_SHAPE_TABLE_LEN_Y] = {
                                                   &g_osc_wave_shape_table_0,
                                                   &g_osc_wave_shape_table_1,
                                                   &g_osc_wave_shape_table_2,
                                                   &g_osc_wave_shape_table_3,
                                                   &g_osc_wave_shape_table_4,
  ...
  INLINE uint32_t get_osc_wave_shape_data(
      const uint16_t (*wave_shape_table)[OSC_WAVE_SHAPE_TABLE_LEN_X]
                                        [OSC_WAVE_SHAPE_TABLE_LEN_Y],
      uint32_t shape, uint16_t saw_number) {
    ...
    int32_t next_data = (*wave_shape_table)[curr_index + 1][saw_number] << 1;
    int32_t data = (curr_data << 8) +
                   (((next_data - curr_data) * next_weight) << 0); // lerp
    return data;
  }

  template <uint8_t N> INLINE int32_t process_osc(int16_t noise_int15) {
    int32_t result = 0;

    int16_t osc1_gain = m_mix_table[(OSC_MIX_TABLE_LENGTH - 1) -
                                    (m_mixer_osc_mix_control_effective >> 1)];
    int16_t osc2_gain = m_mix_table[(m_mixer_osc_mix_control_effective >> 1)];

    m_phase[N] += m_freq[N];
    boolean new_period_osc1 =
        (m_phase[N] & 0x00FFFFFF) <
        m_freq[N]; // crossing the begin of a osc 1 wave, the begin or the
                   // middle of a sub osc wave
    m_wave_table[N] = reinterpret_cast<const int16_t *>(
        (reinterpret_cast<const uintptr_t>(m_wave_table[N]) *
         (1 - new_period_osc1)));
    m_wave_table[N] = reinterpret_cast<const int16_t *>(
        reinterpret_cast<const uint8_t *>(m_wave_table[N]) +
        (reinterpret_cast<const uintptr_t>(m_wave_table_temp[N]) *
         new_period_osc1));

    m_wave_table[N + 12] = reinterpret_cast<const int16_t *>(
        (reinterpret_cast<const uintptr_t>(m_wave_table[N + 12]) *
         (1 - new_period_osc1)));
    m_wave_table[N + 12] = reinterpret_cast<const int16_t *>(
        reinterpret_cast<const uint8_t *>(m_wave_table[N + 12]) +
        (reinterpret_cast<const uintptr_t>(m_wave_table_temp[N + 12]) *
         new_period_osc1));

    m_wave_table[N + 16] = reinterpret_cast<const int16_t *>(
        (reinterpret_cast<const uintptr_t>(m_wave_table[N + 16]) *
         (1 - new_period_osc1)));
    m_wave_table[N + 16] = reinterpret_cast<const int16_t *>(
        reinterpret_cast<const uint8_t *>(m_wave_table[N + 16]) +
        (reinterpret_cast<const uintptr_t>(m_wave_table_temp[N + 16]) *
         new_period_osc1));

    if (m_waveform[0] == WAVEFORM_SINE) {
      // For Sine Wave (wave_3)
      uint16_t osc1_phase_modulation_depth =
          maximum(m_osc1_shape_effective[N] - (128 << 8), 0);
      volatile int32_t phase_modulation_frequency_ratio_candidate =
          (((m_osc1_morph_control_effective + 2) >> 2) << 1) + 2;
      m_osc1_phase_modulation_frequency_ratio[N] =
          (m_osc1_phase_modulation_frequency_ratio[N] * (1 - new_period_osc1)) +
          (phase_modulation_frequency_ratio_candidate * new_period_osc1);

      uint32_t phase_3 = (((m_phase[N] >> 1) & 0x01FFFFFF) *
                          m_osc1_phase_modulation_frequency_ratio[N]) >>
                         1;
      ...
      uint32_t phase_0 =
          m_phase[N] + ((wave_3 * osc1_phase_modulation_depth) >> 4);
      ...
      volatile int32_t phase_modulation_depth =
          maximum(m_osc1_shape_effective[N] - (128 << 8), 0);

      uint32_t freq_shape_morph =
          ((static_cast<int32_t>(
                (m_freq[N] >> 1) *
                g_osc_tune_table[(((phase_modulation_depth + 512) >> 10) + 1 +
                                  128) >>
                                 (8 - OSC_TUNE_TABLE_STEPS_BITS)]) >>
      ...
      int32_t wave_0_1 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - (m_phase_shape_morph[N] * 1) - (phase_shift_base * 3));
      int32_t wave_0_2 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] + (m_phase_shape_morph[N] * 1) + (phase_shift_base * 5));
      int32_t wave_0_3 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - (m_phase_shape_morph[N] * 3) - (phase_shift_base * 5));
      int32_t wave_0_4 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] + (m_phase_shape_morph[N] * 3) + (phase_shift_base * 1));
      int32_t wave_0_5 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - (m_phase_shape_morph[N] * 5) - (phase_shift_base * 1));
      int32_t wave_0_6 = get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] + (m_phase_shape_morph[N] * 5) + (phase_shift_base * 3));

      int32_t multi_saw_mix = (m_osc1_morph_control_effective + 1) >> 1;
      result +=
          ((((multi_saw_mix * (((wave_0_0 + wave_0_1 + wave_0_2 + wave_0_3 +
                                 wave_0_4 + wave_0_5 + wave_0_6)
                                << 1) /
                               5)) +
             ((64 - multi_saw_mix) * wave_0)) >>
            6) *
           osc1_gain * m_osc_level_effective[N]) >>
          10;
    } else if (m_waveform[0] == WAVEFORM_SQUARE) {
      uint32_t shape = maximum(m_osc1_shape_effective[N] - (128 << 8), 0);
      const uint16_t(*wave_shape_table)[OSC_WAVE_SHAPE_TABLE_LEN_X]
                                       [OSC_WAVE_SHAPE_TABLE_LEN_Y] =
                                           &g_osc_sqr_shape_table;

      int32_t wave_0 = +get_wave_level(m_wave_table[N], m_phase[N]);
      int32_t wave_0_0 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 0));
      int32_t wave_0_1 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 1));
      int32_t wave_0_2 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 2));
      int32_t wave_0_3 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 3));
      int32_t wave_0_4 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 4));
      int32_t wave_0_5 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 5));
      int32_t wave_0_6 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 6));
      int32_t wave_0_7 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 7));
      int32_t wave_0_8 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 8));
      int32_t wave_0_9 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 9));
      int32_t wave_0_10 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 10));
      int32_t wave_0_11 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 11));
      int32_t wave_0_12 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 12));
      int32_t wave_0_13 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 13));
      int32_t wave_0_14 = +get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 14));
      int32_t wave_0_15 = -get_wave_level(
          m_wave_table[N + 16],
          m_phase[N] - get_osc_wave_shape_data(wave_shape_table, shape, 15));

      int32_t sqr_sync_mix = (m_osc1_morph_control_effective + 1) >> 1;
      result +=
          ((((sqr_sync_mix *
              (wave_0_0 + wave_0_1 + wave_0_2 + wave_0_3 + wave_0_4 + wave_0_5 +
               wave_0_6 + wave_0_7 + wave_0_8 + wave_0_9 + wave_0_10 +
               wave_0_11 + wave_0_12 + wave_0_13 + wave_0_14 + wave_0_15)) +
             ((64 - sqr_sync_mix) * wave_0)) >>
            6) *
           osc1_gain * m_osc_level_effective[N]) >>
          10;
    } else if (m_waveform[0] == WAVEFORM_1_WAVE_TABLE) {
#if CFG_OSC_MODE == OSC_MODE_TABLE
      // Use pre-generated wave tables for shared OSC/LFO consistency and
      // efficiency m_osc1_morph_control (0-127) selects table
      uint32_t morph = m_osc1_morph_control;
      float t = morph / 127.0f;
      if (t > 1.0f)
        t = 1.0f;
      ...
      // Select mipmap based on note number
      // Assuming note_number is available or derived from freq?
      // m_freq[N] is phase increment.
      // We can use a fixed mipmap for simplicity or calculate it.
      // Or just use the highest quality (0) or iterate based on m_freq[N]
      // For now, let's use mipmap 0 (255 harmonics) or simple selection
      // Better: Use get_wave_table helper but extended for custom tables

      // Since we don't have note_number here easily without re-calculation,
      // let's use the m_wave_table[N] pointer logic if we updated it.
      // But m_wave_table[N] is set to Square wave above.

      // Let's use g_osc_custom_wave_tables_all directly.
      // We need to pick a mipmap.
      // Freq/Rate ratio?
      // Simple approx:
      // uint8_t note = (m_pitch_current[N] >> 24);
      // But m_pitch_current might be modulated.
      // Let's use a safe mipmap index, e.g., 0 (full quality) or middle.
      // Actually, we should really use the proper mipmap.
      // Let's assume note 60 for now to match typical LFO range or just use
      // mipmap 0. Wait, this is OSC, so it needs anti-aliasing!

      uint8_t note = (m_pitch_current[N] >> 24);
      if (note < NOTE_NUMBER_MIN)
        note = NOTE_NUMBER_MIN;
      if (note > NOTE_NUMBER_MAX)
        note = NOTE_NUMBER_MAX;
      uint8_t note_idx = note - NOTE_NUMBER_MIN;

      const int16_t *t0 = g_osc_custom_wave_tables_all[type0][note_idx];
      const int16_t *t1 = g_osc_custom_wave_tables_all[type1][note_idx];

      int32_t val0 = get_wave_level(t0, m_phase[N]);
      int32_t val1 = get_wave_level(t1, m_phase[N]);

      int32_t wave_0 = val0 + (((val1 - val0) * (int)(frac * 256.0f)) >> 8);
      ...
      result += (((64 * (wave_0_0 + wave_0_1 + wave_0_2 + wave_0_3 + wave_0_4 +
                         wave_0_5 + wave_0_6 + wave_0_7 + wave_0_8 + wave_0_9 +
                         wave_0_10 + wave_0_11 + wave_0_12 + wave_0_13 +
                         wave_0_14 + wave_0_15)) >>
                  6) *
                 osc1_gain * m_osc_level_effective[N]) >>
                10;
      ...
      result += ((((wave_3 * osc1_gain * m_osc_level_effective[N]) >> 10) *
                  (((m_osc1_morph_control_effective - 63) >> 1) << 1)) >>
                 6);
    } else {
      int32_t wave_0 = get_wave_level(m_wave_table[N], m_phase[N]);
      result += (wave_0 * osc1_gain * m_osc_level_effective[N]) >> 10;
    ...
    if (m_mixer_noise_sub_osc_control_effective >= 0) {
      ...
      result += (wave_1 * m_mixer_noise_sub_osc_control_effective *
                 m_osc_level_effective[N]) >>
                6;
    } else {
      // Noise (wave_1)
    ...
    m_wave_table[N + 4] = reinterpret_cast<const int16_t *>(
        (reinterpret_cast<const uintptr_t>(m_wave_table[N + 4]) *
         (1 - new_period_osc2)));
    m_wave_table[N + 4] = reinterpret_cast<const int16_t *>(
        reinterpret_cast<const uint8_t *>(m_wave_table[N + 4]) +
        (reinterpret_cast<const uintptr_t>(m_wave_table_temp[N + 4]) *
         new_period_osc2));
  ...
  template <uint8_t N> INLINE void update_pitch_current() {
    if (m_osc_on[N]) {
      if (m_pitch_current[N] <= m_pitch_target[N]) {
        m_pitch_current[N] =
            m_pitch_target[N] -
            multiply_shift_right((m_pitch_target[N] - m_pitch_current[N]) << 2,
                                 m_portamento_coef[N], 32);
      } else {
        m_pitch_current[N] =
            m_pitch_current[N] +
            multiply_shift_right((m_pitch_target[N] - m_pitch_current[N]) << 2,
                                 (1 << 30) - m_portamento_coef[N], 32);
      }
    }
  }
  ...
  INLINE void update_freq_base(int16_t lfo_level, int16_t eg_level) {
    ...
    uint16_t pitch_temp = (64 << 8) + (m_pitch_current[N & 0x03] >> 16) +
                          m_pitch_bend_normalized +
                          ((eg_level * pitch_eg_amt) >> 14);
    ...
    int32_t offset =
        ((static_cast<int32_t>(
              (m_freq_base[N] >> 1) *
              g_osc_tune_table[fine >> (8 - OSC_TUNE_TABLE_STEPS_BITS)]) >>
          OSC_TUNE_DENOMINATOR_BITS) >>
         0)
        << 1;
  ...
  template <uint8_t N> INLINE void update_freq_offset() {
  ...
  INLINE void update_osc1_shape_control_effective() {
    m_osc1_shape_control_effective +=
        (m_osc1_shape_control_effective < m_osc1_shape_control);
    m_osc1_shape_control_effective -=
        (m_osc1_shape_control_effective > m_osc1_shape_control);
  }
  ...
  INLINE void update_osc1_morph_control_effective() {
    m_osc1_morph_control_effective +=
        (m_osc1_morph_control_effective < m_osc1_morph_control);
    m_osc1_morph_control_effective -=
        (m_osc1_morph_control_effective > m_osc1_morph_control);
  }
  ...
  INLINE void update_mixer_control_effective() {
    m_mixer_osc_mix_control_effective +=
        (m_mixer_osc_mix_control_effective < m_mixer_osc_mix_control);
    m_mixer_osc_mix_control_effective -=
        (m_mixer_osc_mix_control_effective > m_mixer_osc_mix_control);
  ...
  INLINE void update_osc1_shape(int16_t lfo_level, int16_t eg_level) {
    volatile int32_t osc1_shape = (128 << 8) +
                                  (m_osc1_shape_control_effective << (8 - 3)) +
                                  ((eg_level * m_shape_eg_amt) >> 5) -
                                  ((lfo_level * m_shape_lfo_amt) >> 5);
    osc1_shape = clamp(osc1_shape, (0 << 8), (256 << 8));
    m_osc1_shape[N] = osc1_shape;
  }

  template <uint8_t N> INLINE void update_osc1_shape_effective() {
    volatile int32_t effective_new =
        clamp(m_osc1_shape[N], (m_osc1_shape_effective[N] - 0x0100),
              (m_osc1_shape_effective[N] + 0x0100));
