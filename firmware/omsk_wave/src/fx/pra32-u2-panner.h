...
public:
  PRA32_U2_Panner()
      : m_pan_table(), m_pan_control(64), m_pan_control_effective(64),
        m_gain_linear_l(16384 << 2), m_gain_linear_r(16384 << 2) {
    for (uint8_t i = 1; i < OSC_PAN_TABLE_LENGTH - 1; ++i) {
      m_pan_table[i] =
          static_cast<int16_t>(
              std::sqrt(2.0) *
              std::sin((PI * (i - 1)) / (2 * (OSC_PAN_TABLE_LENGTH - 1))) *
              (1 << 14))
          << 2;
    }

    m_pan_table[0] = m_pan_table[1];
    m_pan_table[OSC_PAN_TABLE_LENGTH - 1] =
        m_pan_table[OSC_PAN_TABLE_LENGTH - 2];
  }
  ...
  INLINE void process_at_low_rate() { update_gain_effective(); }

  INLINE int32_t process(int32_t audio_input_int24,
                         int32_t &audio_output_r_int24) {
    int32_t audio_output_l_int24 = 0;

    audio_output_l_int24 =
        multiply_shift_right(audio_input_int24, m_gain_linear_l, 16);
    audio_output_r_int24 =
        multiply_shift_right(audio_input_int24, m_gain_linear_r, 16);
